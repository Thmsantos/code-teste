...
const atletaTel = require("../models/telAtletaModels"); // "atletaTel" é uma variável que vai receber o model telefone do atleta
...
class atletaControllers {
  ...
  static async changeUser(req, res) {
    ...
    } else if (tipo_tel_atl == null) {
      res.status(404).json({msg:"Telefone inválido"}); // caso esteja errado, envia outra resposta para o cliente
    }
  ...
  static async viewFile(req, res) {
    ...
    let infosEx = await atletaModels.findAll({ // variavel que guarda as informações do atleta
      raw: true,
      where: { email: email },
    });

    let request = await examesModels.findAll({ // variavel que guarda as informações do exame do atleta
      raw: true,
      where: {
        id_exame_atl: infosEx[0].idatleta,
      },
    });
    console.log(request)
    ...
    if(request.length > 0){
    const medico = await examesModels.sequelize.query(
      `select m.nome from exames e inner join medicos m on m.idmedico = e.id_exame_med where idexame = ${request[0].idexame}`,
      { type: QueryTypes.SELECT }
    ); // variavel que guarda o médico responsável pelo exame do atleta 

      request.map((dados) => {
        (infosEx[0]["idexame"] = dados.idexame),
          (infosEx[0]["tipo"] = dados.tipo),
          (infosEx[0]["descricao"] = dados.descricao),
          (infosEx[0]["observacao"] = dados.observacao);
      }); // adiciona os dados de exame dentro do objeto que contem os dados do atleta

      infosEx[0]["medico"] = medico[0].nome; // adiociona o nome do médico aos dados que serão enviados
      res.send(infosEx); // envia a resposta para o cliente
    }
    else{
      res.send({msg:"Não há exames no momento"})
    }
  ...
  static async alterarPdf(req, res) {
    ...
    if (decisao === false) { // caso falso ele deve excluir o arquivo     
      ...
      res.send({msg:`http://localhost:3010/guia_exame-${request[0].idexame}.pdf`}); // envia a resposta para o front
    }
  };
  ...
  static async sendPdf(req, res) {
    ...
    await examesModels.update( // Realiza a alteração no banco de dados
      { pdfatleta: pdf },
      { where: { id_exame_atl: atleta.idatleta } }
